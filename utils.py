...
def load_feat(d, rand_de=128, rand_dn=128):
    node_feats = None
    ...
    if rand_de > 0:
        if d == 'LASTFM':
            ...
            edge_feats = torch.randn(157474, rand_de)
        elif d == 'REDDIT':
            edge_feats = torch.randn(672447, rand_de)
    if rand_dn > 0:
        if d == 'LASTFM':
...
def mfgs_fetch(ret, mfgs, hist, layer, fetchCPU=False):
    sample_id_t=[]
    ctData = []
    for i in range(hist):
        sample_id_t.append([])
        ctData.append([])
        if fetchCPU:
            sample_id_t[i].append(torch.from_numpy(ret[(layer-1)*hist+i].nodes()))
        else:
            sample_id_t[i].append(mfgs[0][0].srcdata['ID'])
        for idx in range(layer):
            lid = i+(idx)*hist
            if fetchCPU:
                sample_id_t[i].append(torch.from_numpy(ret[lid].eid())) # USE
            else:
                sample_id_t[i].append(mfgs[-1-idx][0].edata['ID']) # USE
            ctData[i].append(ret[lid].uniE())
            ctData[i].append(ret[lid].invE())
    return sample_id_t, ctData
    ...
    if node_feats is not None:
        for b in mfgs[0]:
            if pinned:
                ...
                t_s1 = time.time()
                # import pdb; pdb.set_trace()
                srch = node_feats[b.srcdata['ID'].long().to(node_feats.device)].float()
                # import pdb; pdb.set_trace()
                t_s2 = time.time()
                if edge_feats.device.type == 'cuda':
                    b.srcdata['h'] = srch
                else:
                    b.srcdata['h'] = srch.cuda()
                t_cuda += time.time() - t_s2
                # import pdb; pdb.set_trace()
                t_idx += t_s2 - t_s1
            ...
            for b in mfg:
                if b.num_src_nodes() > b.num_dst_nodes():
                    if pinned:
                        ...
                        t_s1 = time.time()
                        srch = edge_feats[b.edata['ID'].long().to(edge_feats.device)].float()
                        t_s2 = time.time()
                        if edge_feats.device.type == 'cuda':
                            b.edata['f'] = srch
                        else:
...
def prepare_input_slim_cache(mfgs, dst_feats):
    t_idx = 0
    t_cuda = 0
    i = 0
    if dst_feats[0][0] is not None:
        for idx,b in enumerate(mfgs[0]):
            b.srcdata['h'] = dst_feats[idx][0]
    if dst_feats[0][1] is not None:
        for ij, mfg in enumerate(mfgs):
            for idx,b in enumerate(mfg):
                if b.num_src_nodes() > b.num_dst_nodes():
                    if ij == 0 and len(mfgs)==2:
                        b.edata['f'] = dst_feats[idx][2]
                    else:
                        b.edata['f'] = dst_feats[idx][1]
    return mfgs
