...
class MailBox():
    ...
    def prep_input_mails(self, mfg, CTdata=None, use_pinned_buffers=False):
        for i, b in enumerate(mfg):
            if use_pinned_buffers:
                ...
                b.srcdata['mem'] = self.node_memory[b.srcdata['ID'].long().to(self.node_memory.device)].cuda()
                b.srcdata['mem_ts'] = self.node_memory_ts[b.srcdata['ID'].long().to(self.node_memory_ts.device)].cuda()
                b.srcdata['mem_input'] = self.mailbox[b.srcdata['ID'].long().to(self.mailbox.device)].cuda().reshape(b.srcdata['ID'].shape[0], -1)
                b.srcdata['mail_ts'] = self.mailbox_ts[b.srcdata['ID'].long().to(self.mailbox_ts.device)].cuda()

    def prep_input_mails_sc(self, mfg, NODE_UV=None):
        for i, b in enumerate(mfg):
            if NODE_UV is None:
                NODE_UV=[None, None]
                NODE_UV[0], NODE_UV[1] = torch.unique(b.srcdata['ID'], return_inverse=True)
            if(NODE_UV[0].device.type=='cuda'):
                NODE_UV[0] = NODE_UV[0].cpu()
            b.srcdata['mem']  = self.node_memory[NODE_UV[0].long()].cuda()[NODE_UV[1]]
            b.srcdata['mem_ts'] = self.node_memory_ts[NODE_UV[0].long()].cuda()[NODE_UV[1]]
            b.srcdata['mem_input'] = self.mailbox[NODE_UV[0]].cuda()[NODE_UV[1]].reshape(b.srcdata['ID'].shape[0], -1)
            b.srcdata['mail_ts'] = self.mailbox_ts[NODE_UV[0].long()].cuda()[NODE_UV[1]]
        return NODE_UV
    ...
    def update_mailbox(self, nid, memory, root_nodes, ts, edge_feats, block, neg_samples=1):
        with torch.no_grad():
            ...
            if edge_feats is not None:
                edge_feats = edge_feats.to(memory.device)
            if block is not None:
                block = block.to(self.device)
            # TGN/JODIE
            if self.memory_param['deliver_to'] == 'self':
                src = torch.from_numpy(root_nodes[:num_true_edges])
                dst = torch.from_numpy(root_nodes[num_true_edges:num_true_edges * 2])
    ...
    def push_update_mailbox(self, nid, memory, root_nodes, ts, edge_feats, edges=None, uni_node=None, inv_node=None, neg_samples=1):
        with torch.no_grad():
            num_true_edges = root_nodes.shape[0] // (neg_samples + 2)
            # TGN/JODIE
            if self.memory_param['deliver_to'] == 'self':
            ...
            if self.memory_param['combine_node_feature']:
                if self.dim_node_feat > 0:
                    if self.dim_node_feat == self.dim_hid:
                        ...
                        b.srcdata['h'] = updated_memory + self.node_feat_map(b.srcdata['h']) # TGN may error when GDELT
                else:
                    b.srcdata['h'] = updated_memory
