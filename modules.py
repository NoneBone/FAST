...
class GeneralModel(torch.nn.Module):
    ...
    def forward(self, mfgs, neg_samples=1):
        ...
        for l in range(self.gnn_param['layer']):
            # triList[l] -> [uniSrcList, uniTsList, invList]
            for h in range(self.sample_param['history']):
                nsysForward.time_push("t_halfForward")
                fm.layerAllNum = self.gnn_param['layer']
                rst = self.layers['l' + str(l) + 'h' + str(h)](mfgs[l][h], l, h)  # this one
